	...
	\section{Analysis}
		\subsection{Model Comparison}
			...
			*  
			...
			We then make a linear fit using weighted least squares, where  weight associated to each entry is proportional to the inverse of the variance, that only depends on $m$.

			We consider different models and report their AIC values in table \ref{tab:WLS_AIC}. As we are only interested in their relative values, the minimimum AIC value among the models compared is substracted from all the other AIC values. 
			...
			\begin{table}[htb]
				...
				\midrule
				\texttt{1} & 159.76 \\
				\texttt{1 + C(person)} & 12.89 \\
				\texttt{1 + C(person) + agg} & 0.00 \\
				\texttt{1 + C(person) + agg + C(coin)} & 29.21 \\
				\texttt{1 + C(person) + agg + C(person):C(coin)} & 123.98 \\
				\bottomrule
				\end{tabular}
			\end{table}
